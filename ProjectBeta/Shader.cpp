...
void Shader::loadVertex(const char* shaderPath)
{
...
void Shader::loadFragment(const char* shaderPath)
{
    GLint Result = GL_FALSE;
    int InfoLogLength;
...
void Shader::loadGeometry(const char* shaderPath)
{
    GLint Result = GL_FALSE;
    int InfoLogLength;
...
void Shader::setUniform(const char* name, bool& value)
{
    if (uniformOffset_.find(name) == uniformOffset_.end()) {
        ...
        uniformOffset_[name] = glGetUniformLocation(program_, name);
    }
...
void Shader::setTexture3D(const char* name, GLuint& value)
{
    if (textureUniformOffset_.find(name) == textureUniformOffset_.end()) {

        TextureUniform uniform;
        uniform.offset = glGetUniformLocation(program_, name);
...
void Shader::setTexture2D(const char* name, GLuint& value)
{
    if (textureUniformOffset_.find(name) == textureUniformOffset_.end()) {
